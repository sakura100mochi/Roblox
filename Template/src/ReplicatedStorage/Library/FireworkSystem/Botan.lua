...
--Return Value	: none
function Botan.launch(Start_CFrame, Color, ExplodeTime, ExplodeSpeed)
	local firework = require(game:GetService("ReplicatedStorage").Shared.Library).firework

	if Start_CFrame == nil then
		warn("ERROR: no argument [FireworkSystem.Botan]")
		return
	end
	...
	for i = 1, 5, 1 do
		local particle = makeBotanParticle(Nobori, ColorSequence.new(Color), NumberRange.new(ExplodeTime), ExplodeSpeed)
...
function Botan.AutoSystem(Start_CFrame)
	...
	while true do
		for i = 1, math.random(3, 5), 1 do
			...
			task.spawn(function()Botan.launch(Start_CFrame, Color, ExplodeTime, ExplodeSpeed)end)
		end
		task.wait(2)
	end
end
